...
function fetchGradedata() {
    // This fuction will query the PostgreSQL database and return grade data 
    console.log("fetching grade data ...");
    // Creat a new request for HTTP data
    let xhr = new XMLHttpRequest();
    // This is the request changes status, we run this anonymous function 
    xhr.onreadystatechange = function(){
        ...
        if (xhr.readyState === xhr.DONE){
            ...
            // and then call the fucnction to update the HTML with our data
...
function populateGradebook(data) {
    ...
    console.log("populating gardebook with data:", data);
    let tableElm = document.getElementById("gradebook"); // Get the gradebook table element
        data.forEach(function(assignment){ // For each row of data we're passed in 
            ...
            columns.name = document.createElement('td'); // The first column table data will be the name 
            columns.name.appendChild( 
                // Concatenate the full name:  "last_name, first_name"
                document.createTextNode(assignment.last_nmae + ", " + assignment.first_name)
            )
